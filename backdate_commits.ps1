...
$realFiles = @(
    ...
    "apps\web\src\app\register\police\page.tsx",
    ...
    "apps\web\src\app\police\pending\page.tsx",
    ...
    "services\api\src\services\abuser.service.ts",
...
$endDate   = [datetime]"2026-07-16"

# Helper to map commit message to a file path
function Get-TargetFileForMessage($msg, $realFiles) {
    $msgLower = $msg.ToLower()
    
    # Define mapping rules based on keywords
    $mapping = [ordered]@{
        "auth" = "services\api\src\services\auth.service.ts"
        "jwt" = "services\api\src\services\auth.service.ts"
        "token" = "services\api\src\services\auth.service.ts"
        "login" = "services\api\src\services\auth.service.ts"
        "biometric" = "services\api\src\services\auth.service.ts"
        
        "witness" = "services\api\src\services\witness.service.ts"
        "evidence" = "services\api\src\services\evidence.service.ts"
        "abuser" = "services\api\src\services\abuser.service.ts"
        
        "case" = "services\api\src\services\case.service.ts"
        "incident" = "services\api\src\services\case.service.ts"
        
        "sos" = "services\api\src\services\emergency-contact.service.ts"
        "contact" = "services\api\src\services\emergency-contact.service.ts"
        "emergency" = "services\api\src\services\emergency-contact.service.ts"
        
        "admin" = "services\api\src\services\admin.service.ts"
        "analytics" = "services\api\src\services\analytics.service.ts"
        "report" = "services\api\src\services\analytics.service.ts"
        ...
        "ai" = "services\ai\main.py"
        "python" = "services\ai\main.py"
        "matching" = "services\ai\matching.py"
        ...
        "expo" = "apps\mobile\lib\api.ts"
        "haptic" = "apps\mobile\lib\api.ts"
        "screen" = "apps\mobile\lib\api.ts"
        
        "web" = "apps\web\src\lib\api.ts"
        "next" = "apps\web\src\lib\api.ts"
        "layout" = "apps\web\src\lib\api.ts"
        "dashboard" = "apps\web\src\lib\api.ts"
    }
    
    foreach ($key in $mapping.Keys) {
        if ($msgLower.Contains($key)) {
            $candidate = $mapping[$key]
            if ($realFiles -contains $candidate) {
                return $candidate
            }
        }
    }
    
    # Fallback to random file
    if ($realFiles.Count -gt 0) {
        return $realFiles[(Get-Random -Minimum 0 -Maximum $realFiles.Count)]
    }
    return "README.md"
}

# Helper to clean up all commit comments from a file
function Clean-CommitComments($fullPath) {
    if (Test-Path $fullPath) {
        $content = Get-Content $fullPath -Raw -Encoding utf8
        $pattern = "(?m)^\s*(//|#|/\*|<!--)\s*Commit:.*$"
        $cleanContent = $content -replace $pattern, ""
        $cleanContent = $cleanContent.TrimEnd() + "`n"
        [System.IO.File]::WriteAllText($fullPath, $cleanContent, [System.Text.UTF8Encoding]::new($false))
    }
}

# Helper to append comment based on file extension
function Add-CommentToFile($fullPath, $msg, $dateIso) {
    $ext = [System.IO.Path]::GetExtension($fullPath).ToLower()
    $comment = ""
    
    switch ($ext) {
        ".ts"   { $comment = "`n// Commit: $msg - $dateIso`n" }
        ".tsx"  { $comment = "`n// Commit: $msg - $dateIso`n" }
        ".js"   { $comment = "`n// Commit: $msg - $dateIso`n" }
        ".jsx"  { $comment = "`n// Commit: $msg - $dateIso`n" }
        ".py"   { $comment = "`n# Commit: $msg - $dateIso`n" }
        ".prisma" { $comment = "`n// Commit: $msg - $dateIso`n" }
        ".yml"  { $comment = "`n# Commit: $msg - $dateIso`n" }
        ".yaml" { $comment = "`n# Commit: $msg - $dateIso`n" }
        ".css"  { $comment = "`n/* Commit: $msg - $dateIso */`n" }
        ".md"   { $comment = "`n<!-- Commit: $msg - $dateIso -->`n" }
        default { $comment = "`n# Commit: $msg - $dateIso`n" }
    }
    
    [System.IO.File]::AppendAllText($fullPath, $comment, [System.Text.UTF8Encoding]::new($false))
}

# Clean all files first to start fresh
Write-Host "Cleaning up old commit comments from project files..."
foreach ($file in $realFiles) {
    $fullPath = Join-Path $repoPath $file
    Clean-CommitComments $fullPath
}
...
while ($current -le $endDate) {
    ...
    $numCommits = Get-Random -Minimum 5 -Maximum 16
    ...
    for ($i = 1; $i -le $numCommits; $i++) {
        ...
        if ($isFirstCommit) {
            # Stage ALL files for the very first commit to capture base project files
            git add .
            $isFirstCommit = $false
        } else {
            # Find the most meaningful target file for this message
            $targetFile = Get-TargetFileForMessage $msg $realFiles
            # Avoid .json files (like package.json)
            if ($targetFile -like "*.json") {
                $targetFile = "README.md"
            }
            $fullPath = Join-Path $repoPath $targetFile

            if (Test-Path $fullPath) {
                # Add a meaningful comment related to this commit
                Add-CommentToFile $fullPath $msg $dateIso
                git add $targetFile
            }
        }
